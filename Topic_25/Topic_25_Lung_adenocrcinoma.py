...
def main():
	...
	files = glob.glob('Topic_25_Relevant/*')
	for f in files:
		os.remove(f)

	files = glob.glob('Topic_25_Irrelevant/*')
	for f in files:
		os.remove(f)
	pre_score = 0
	orderNum = 0
	...
	for i in range(0,len(file_score_array)):
		src = "Lung adenocrcinoma/" + file_name_array[i]
		if file_score_array[i] > 0:
			dst = "Topic_25_Relevant/" + file_name_array[i]
			shutil.copyfile(src,dst)
		else:
			dst = "Topic_25_Irrelevant/" + file_name_array[i]
			shutil.copyfile(src,dst)
